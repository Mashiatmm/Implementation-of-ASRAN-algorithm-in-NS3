...
#-------------------------------------------------varying flows-----------------------------------------------------
filenames = ['nodes_10_flows_10', 'nodes_10_flows_20', 'nodes_10_flows_30', 'nodes_10_flows_40', 'nodes_10_flows_50']
...
for param, values in Parameters.items():
    ...
    font['size']=18
    plt.title("Nodes = 10, Flow vs "+param,fontdict=font)
    plt.grid()
...
#-------------------------------------------------varying speed-----------------------------------------------------
filenames = ['speed_5', 'speed_10', 'speed_15', 'speed_20', 'speed_25' ]
...
for param, values in Parameters.items():
    ...
    if param == "Total sent packets ":
    	...
    	continue
    k=5
    with open("speed_plots/"+param+".csv", "w") as outfile:
        csv_writer = csv.writer(outfile)
        csv_writer.writerow(['Speed', param])
        for val in values :
            csv_writer.writerow([k,val])
            k+=5
    df = pd.read_csv("speed_plots/"+param+".csv")
    
   
    
    font['size']=16
    figure(figsize=(16, 12), dpi=80)
    plt.scatter(df['Speed'], df[param], color ='maroon')
    plt.plot(df['Speed'], df[param], color ='maroon')
    # plt.bar(df['Packets'], df[param], color ='maroon', width = 5.0)
    plt.xticks(np.arange(0,26,5)) 
    plt.xlabel('Speed',fontdict=font)  # Add an x-label to the axes.
    plt.ylabel(param,fontdict=font)
    font['size']=18
    plt.title("Speed vs "+param,fontdict=font)
    plt.grid()

    plt.savefig("speed_plots/"+param+".jpg")
    plt.clf()
    if os.path.exists(param+".csv"):
        os.remove(param+".csv")
